...
namespace shadow {
	...
	public:
		...
		template<typename F, typename... Args>
		static void commit(F&& f, Args&&... args) {
			TaskFactory::instance().thread_pool_.commit(std::forward<F>(f), std::forward<Args>(args)...);
		}

	private:
		TaskFactory() :thread_pool_(10) {}
		TaskFactory(const TaskFactory& oth) = delete;
		TaskFactory& operator = (const TaskFactory& oth) = delete;

		static TaskFactory& instance() {
			static TaskFactory tf;
			return tf;
		}
